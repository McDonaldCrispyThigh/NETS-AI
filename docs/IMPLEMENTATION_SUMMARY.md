...
[_][-] Validate: python scripts/validate_environment.py
...
[_][-] Deduplication: Unique place_id extraction

Step 2: Data Collection (per business)
[|][-] Google Maps Details API call
 [_][-] Fields: name, address, phone, website, rating, reviews, hours
[|][-] Website Status Check
 [_][-] HTTP request with 10-second timeout
...
class PGBDM:
 ...
 Math:
 Entry_count ~ Poisson(_{zip,category})
 = exp(0 + 1*median_income + 2*population_density)
 """
 ...
 Math:
 h(t|X) = h0(t) * exp(1*rating_decline + 2*review_velocity + ...)
 
 Interpretation:
 - > 0: Increases closure hazard
 - exp() = hazard ratio
 """
