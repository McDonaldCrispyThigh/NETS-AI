"""
NETS-Enhancement Complete Data Collection Pipeline
Integrates: Google Maps + Wayback Machine + LLM Analysis
"""
...
# Setup logger
logger = setup_logger("NETS-Enhancement-Pipeline")
...
def collect_business_data(place_id: str, maps_agent: GoogleMapsAgent) -> dict:
    ...
    if hasattr(maps_agent, 'get_place_reviews_timeseries'):
        try:
            ...
            if reviews_timeseries:
                stats = maps_agent.extract_review_statistics(
                    reviews_timeseries)
                ...
                logger.info(
    f" Saved {
        len(reviews_timeseries)} reviews to {
            review_file.name}")
    ...
    if not reviews_timeseries and reviews:
        ...
        if timestamps:
            data['last_review_date'] = datetime.fromtimestamp(
                max(timestamps)).strftime('%Y-%m-%d')
            data['oldest_review_date'] = datetime.fromtimestamp(
...
def enhance_with_wayback(
    business_data: dict,
     wayback_agent: WaybackAgent) -> dict:
    ...
    if first:
        business_data['wayback_first_snapshot'] = first['date'].strftime(
            '%Y-%m-%d')
    ...
    if last:
        business_data['wayback_last_snapshot'] = last['date'].strftime(
            '%Y-%m-%d')
...
def enhance_with_gpt(
    ...
    business_data['ai_risk_factors'] = ", ".join(
        status_result.get('risk_factors', []))
    ...
    business_data['ai_employees_estimate'] = employment_result.get(
        'best_estimate')
    business_data['ai_employees_confidence'] = employment_result.get(
        'confidence')
    ...
    business_data['ai_naics_suggested'] = naics_result.get(
        'actual_naics_suggestion')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="NETS-Enhancement Complete Pipeline: Google Maps + Wayback + LLM Analysis"
    )
    ...
    logger.info(f"NETS-Enhancement Pipeline: {args.task} in {args.city}")
    ...
    output_file = output_dir / f"nets_enhanced_{args.city}_{args.task}_{timestamp}.csv"
    ...
    if 'ai_status' in df.columns:
        logger.info(f" Active (LLM): {df['ai_status'].value_counts().get('Active', 0)}")
        logger.info(f" Inactive (LLM): {df['ai_status'].value_counts().get('Inactive', 0)}")
